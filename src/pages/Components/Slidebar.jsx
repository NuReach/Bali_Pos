...
export default function Slidebar() {
  ...
  const route = "Menu";
  return (
    <div className='flex flex-col gap-6 w-36 border-r min-h-screen shadow-lg'>
        <section className='flex flex-col gap-6 p-6'>
            {
                pages.map((item,i)=>(
                    <button key={i} className={route == item.title ? ' bg-yellow-700 text-white p-3 flex flex-col gap-1 rounded-lg justify-center items-center' : '  text-gray-700 p-3 flex flex-col gap-1 rounded-lg justify-center items-center'}>
